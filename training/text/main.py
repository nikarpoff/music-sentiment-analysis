...
from text.utils import train_text_sentiment_model
from utils.utils import get_params_count
...
def cli_arguments_preprocess() -> str:
    ...
    parser.add_argument("--cpath", required=True,
                        help="Path to cash downloaded dataset")
    ...
    return args.spath, args.cpath, args.task



if __name__ == "__main__":
    source_path, cash_path, task_type = cli_arguments_preprocess()
    load_dotenv()
    ...
    epochs = int(os.getenv("TEXTR_EPOCHS", 5))
    ...
    dataloader = TextSentimentDataLoader(
        source_dataset_path=source_path,
        dataset_cashed_path=cash_path,
    ...
    model = TextSentimentTransformer(output_channels=2, dropout=dropout, whisper_model_name=whisper_model_name, device=device).to(device)
    ...
    if task_type == "train":
        ...
        val_loader = dataloader.get_dataloader("validation")
